...
class BartApi():
  ...
  def _parse_response(self,raw_xml):
  ...
  def _get_xml(self,url):
    ...
    xml = self._parse_response(raw_response)
    return xml

  def number_of_trains(self):
    """ Returns the number of trains currently running"""
    xml = self._get_xml("http://api.bart.gov/api/bsa.aspx?cmd=count&key=%s" % (self.api_key))
    train_count = xml.xpath('traincount')[0].text
    return train_count

  def elevator_status(self):
    """ Returns the status of the elevators in the BART system """
    xml = self._get_xml("http://api.bart.gov/api/bsa.aspx?cmd=elev&key=%s" % (self.api_key))
    elevator_status = xml.xpath('bsa')[0].xpath('description')[0].text
    return elevator_status

  def get_stations(self):
    """ Returns a list of BART stations """
    xml = self._get_xml("http://api.bart.gov/api/stn.aspx?cmd=stns&key=%s" % (self.api_key))
  ...
  def bsa(self, stn="ALL"):
    xml = self._get_xml("http://api.bart.gov/api/bsa.aspx?cmd=bsa&orig=%s&key=%s" % (stn,self.api_key))
    return xml.find(".//description").text

  def station_info(self, station):
    xml = self._get_xml("http://api.bart.gov/api/stn.aspx?cmd=stninfo&orig=%s&key=%s" % (station,self.api_key))
    raw_station = xml.find(".//station")
    return dict(((elt.tag,elt.text) for elt in raw_station))

  def station_access(self, station, legend="1"):
    xml = self._get_xml("http://api.bart.gov/api/stn.aspx?cmd=stnaccess&orig=%s&key=%s&l=%s" % (station,self.api_key,legend))
    xml = xml.find(".//station") 
    return dict(((elt.tag,elt.text) for elt in xml))

  def etd(self, station="ALL", platform=None, direction=None):
    if station == "ALL":
      xml = self._get_xml("http://api.bart.gov/api/etd.aspx?cmd=etd&orig=ALL&key=%s" % (self.api_key))
    elif platform is None:
      xml = self._get_xml("http://api.bart.gov/api/etd.aspx?cmd=etd&orig=%s&direction=%s&key=%s" % (station,direction,self.api_key))
    elif direction is None:
      url = "http://api.bart.gov/api/etd.aspx?cmd=etd&orig=%s&platform=%s&key=%s" % (station,platform,self.api_key)
    else:
      xml = self._get_xml("http://api.bart.gov/api/etd.aspx?cmd=etd&orig=%s&platform=%s&direction=%s&key=%s" % (station,platform,direction,self.api_key))
  ...
  def route_info(self, route="all", sched=None, date="today"):
    if sched is None:
      xml = self._get_xml("http://api.bart.gov/api/route.aspx?cmd=routeinfo&route=%s&date=%s&key=%s" % (route,date,self.api_key))
    else:
      xml = self._get_xml("http://api.bart.gov/api/route.aspx?cmd=routeinfo&route=%s&sched=%s&date=%s&key=%s" % (route,sched,date,self.api_key))
  ...
  def get_holidays(self):
    xml = self._get_xml("http://api.bart.gov/api/sched.aspx?cmd=holiday&key=%s" % (self.api_key))
  ...
  def get_schedules(self):
    xml = self._get_xml("http://api.bart.gov/api/sched.aspx?cmd=scheds&key=%s" % (self.api_key))
  ...
  #  xml = self._get_xml("http://api.bart.gov/api/sched.aspx?cmd=special&key=%s&l=%s" % (self.api_key,legend))
  ...
  def get_station_schedule(self, station):
    xml = self._get_xml("http://api.bart.gov/api/sched.aspx?cmd=stnsched&orig=%s&key=%s" % (station,self.api_key))
  ...
  #  if not sched=='':
  #    xml = self._get_xml("http://api.bart.gov/api/sched.aspx?cmd=routesched&sched=%s&key=%s&l=%s" % (sched,self.api_key,legend))
  #  elif sched == '':
  #    xml = self._get_xml("http://api.bart.gov/api/sched.aspx?cmd=routesched&date=%s&key=%s&l=%s" % (date,self.api_key,legend))
  ...
  def get_fare(self, orig, dest):
    xml = self._get_xml("http://api.bart.gov/api/sched.aspx?cmd=fare&orig=%s&dest=%s&key=%s" % (orig,dest,self.api_key))
